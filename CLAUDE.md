...
After build: `node bin/brewtui-bar.js` or `./bin/brewtui-bar.js` launches the TUI.

**BrewTUI-Bar (menubar/):**
```bash
cd menubar && tuist generate                                            # Regenerate Xcode workspace
xcodebuild -workspace BrewTUI-Bar.xcworkspace -scheme BrewTUI-Bar \
  -configuration Debug build CODE_SIGNING_ALLOWED=NO                    # Build the menubar app
```

CLI subcommands (run without launching TUI):
```bash
brewtui-bar activate <key>    # Activate Pro license via Polar
brewtui-bar revalidate        # Revalidate the current Pro license
brewtui-bar deactivate        # Deactivate license on this machine
brewtui-bar status            # Show evaluated license status
brewtui-bar install-brewtui-bar       # Download & install BrewTUI-Bar menubar app
brewtui-bar install-brewtui-bar --force  # Reinstall BrewTUI-Bar
brewtui-bar uninstall-brewtui-bar     # Remove BrewTUI-Bar from /Applications
...
**Swift tests (BrewTUI-Bar):**
```bash
xcodebuild test -workspace menubar/BrewTUI-Bar.xcworkspace \
  -scheme BrewTUI-Bar -destination 'platform=macOS,arch=arm64' \
  CODE_SIGNING_ALLOWED=NO
```

## Architecture

**BrewTUI-Bar** is a visual TUI for Homebrew built with React 18 + Ink 5.x (terminal renderer). ESM-only (`"type": "module"`), TypeScript strict mode. Requires Node ≥ 22.
...
- **Licensing (4.0.0+):** `~/.brewtui-bar/license.json` carries an Ed25519-signed envelope `{version:2, license, sig}` issued by `brewtui-api` (NAS). Clients verify offline with the embedded public key `oHtzyU7…wq0=`. Pre-4.0.0 AES-GCM envelopes (v1) are rejected — user runs `brewtui-bar revalidate` once to migrate. Revalidates every 24h with 7-day offline grace.
- **Machine binding:** License envelope includes `machineId` from `~/.brewtui-bar/machine-id` (UUID generated on first activation). Prevents license portability between devices.
...
- **Data directory:** `~/.brewtui-bar/` managed by `src/lib/data-dir.ts` (license.json, machine-id, profiles/, history.json)
...
- BrewTUI-Bar requires BrewTUI-Bar installed; checked on launch via `which brewtui-bar` and known paths.
...
- **brewtui-bar** — CLI command, npm package name, filesystem paths (`~/.brewtui-bar/`) — unchanged
- **BrewTUI-Bar.app** / **BrewTUI-Bar** process name — on-disk bundle + `pgrep` target (legacy hyphenation kept for compatibility; do not rename without a migration release)
- **brewtui-bar** — cask name in the tap, `install-brewtui-bar` CLI subcommand
...
10. If the view triggers a `brew upgrade`/`install`/`uninstall`, call `writeLastAction()` from `src/lib/data-dir.ts` after the stream succeeds so BrewTUI-Bar's banner reflects it.

## Internationalization (i18n)

Both the TypeScript TUI and BrewTUI-Bar support English (en) and Spanish (es).
...
### Swift BrewTUI-Bar
- **String Catalog:** `menubar/BrewTUI-Bar/Resources/Localizable.xcstrings` (en + es)
...
2. **BrewTUI-Bar:** For SwiftUI views, just write `Text("New string")`. For non-SwiftUI, use `String(localized: "New string")`. Add Spanish translation in `.xcstrings`.
...
- **BrewTUI-Bar:** Test target `BrewTUI-BarTests` defined in `Project.swift`. 30 tests across 8 suites (Swift Testing `@Suite` / `@Test`) in `menubar/BrewTUI-BarTests/Sources/{BrewTUIBarTests,ServiceTests}.swift`. Run with the `xcodebuild test` command in the Commands section.
