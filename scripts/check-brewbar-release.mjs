...
// de package.json contenga BrewBar.app.zip + .sha256. Sin esto, `npm publish`
// puede liberar un brew-tui cuyo `install-brewbar --force` apunta a una URL
...
if (process.env.SKIP_BREWBAR_CHECK === '1') {
  console.warn('⚠  SKIP_BREWBAR_CHECK=1 — salto verificación de assets BrewBar.');
  process.exit(0);
}
...
const required = ['BrewBar.app.zip', 'BrewBar.app.zip.sha256'];
...
const fail = (msg) => {
  console.error(`✘ ${msg}`);
  console.error('  Runbook: notarizar BrewBar y subir assets ANTES de npm publish.');
  ...
  console.error(`    3. gh release upload ${tag} menubar/build/BrewBar.app.zip menubar/build/BrewBar.app.zip.sha256 --repo ${repo}`);
  console.error('  Bypass de emergencia (no recomendado): SKIP_BREWBAR_CHECK=1 npm publish');
  process.exit(1);
};
...
console.log(`✓ Release ${tag} tiene los assets BrewBar (${required.join(', ')}).`);
