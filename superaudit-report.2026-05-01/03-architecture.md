...
| Licencia (TUI) | `license-store.ts` → `~/.brewtui-bar/license.json` | `isPro()`, `isTeam()`, degradacion | Politica de gracia diverge con BrewBar | Unificar logica |
| Licencia (BrewBar) | `LicenseChecker.swift` → `~/.brewtui-bar/license.json` | `canUpgrade` en AppState | Umbral 30 dias vs 7 dias TS | Alinear umbral |
...
| CVE alerts (BrewBar) | `SecurityMonitor` actor → `~/.brewtui-bar/cve-cache.json` | `AppState.cveAlerts`, badge critico | Es el unico writer del archivo | Documentar exclusividad |
