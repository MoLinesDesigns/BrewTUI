...
| Target `brewtui-bar` (npm CLI) | Conforme | — | `package.json`: bin, main, files, engines, prepublishOnly todos correctamente configurados | — |
...
* [ ] Configuracion local no filtrada al repo — **Media**: `dist-standalone/brewtui-bar-bun` (65 MB, binario Mach-O arm64) comprometido en git y no gitignoreado
...
| `dist-standalone/brewtui-bar-bun` comprometido en git | No conforme | Media | `git ls-files dist-standalone/brewtui-bar-bun`: archivo tracked. Binario Mach-O arm64 de 65 MB. No figura en `.gitignore` ni en `.npmignore`. No documentado en README. No hay fuga incremental de secretos (el bundle npm contiene los mismos literales), pero el artefacto ocupa 65 MB en el historial git sin procedencia verificable | Anadir `dist-standalone/` a `.gitignore` y `.npmignore`. Ejecutar `git rm --cached dist-standalone/brewtui-bar-bun`. Si se distribuye, hacerlo via GitHub Releases con SHA256 verificable |
...
| Homebrew Formula `brewtui-bar.rb` en version `0.5.3` | No conforme | Alta | `homebrew/Formula/brewtui-bar.rb:3`: `url` apunta a `brewtui-bar-0.5.3.tgz`. Desincronizacion de 2 versiones menores respecto a `0.6.1`. Usuarios que instalen via `brew install` reciben version sin los fixes de `0.6.0` y `0.6.1` | Actualizar `url`, `sha256` y el test en `brewtui-bar.rb` a `0.6.1`. Incluir este paso en el checklist de `publish-all.sh` |
...
| Licencia MIT verificada | Conforme | — | `LICENSE:1`: `MIT License`; `package.json:44`: `"license": "MIT"`; `homebrew/Formula/brewtui-bar.rb:6`: `license "MIT"` — coherentes | — |
...
| `.gitattributes` ausente | No conforme | Baja | `find /Volumes/SSD/Projects/BrewTUI-Bar -maxdepth 2 -name ".gitattributes"`: no encontrado. Sin `.gitattributes`, no hay normalizacion de line endings, ni marcadores de binary para PNG, ni `export-ignore` para tarballs de GitHub | Crear `.gitattributes` con: `* text=auto`, binarios (`*.png`, `*.p12`) como `binary`, `export-ignore` para `scripts/`, `superaudit-report/`, `screenshots/` |
...
6. **Homebrew Formula y Cask desactualizados** (seccion 2.4) — `brewtui-bar.rb:3` en `0.5.3` y `brewbar.rb:2` en `0.1.0` vs version publicada `0.6.1`.
